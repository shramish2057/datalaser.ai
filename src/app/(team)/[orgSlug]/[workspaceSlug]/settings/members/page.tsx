    ...
    <WsSettingsShell orgSlug={orgSlug} workspaceSlug={workspaceSlug}>
      {loading ? <div className="space-y-3"><div className="h-10 rounded-dl-md dl-shimmer" /><div className="h-10 rounded-dl-md dl-shimmer" /></div> : <>
        <div className="flex items-center justify-between mb-6">
          ...
          {availableOrgMembers.length > 0 && <button onClick={() => setAddOpen(true)} className="dl-btn-primary"><UserPlus size={14} /> Add member</button>}
        </div>
        ...
        <Dialog open={addOpen} onOpenChange={setAddOpen}>
          <DialogContent className="bg-dl-bg border border-dl-border rounded-dl-lg shadow-dl-lg p-0 max-w-md">
            <DialogHeader className="p-6 pb-0"><DialogTitle className="text-dl-xl font-black text-dl-text-dark">Add member to workspace</DialogTitle></DialogHeader>
            <div className="p-6 pt-4 space-y-4">
              <div><label className="dl-label">Select org member</label><select className="dl-input" value={selectedUser} onChange={e => setSelectedUser(e.target.value)}>
                <option value="">{t("common.choose")}</option>
                {availableOrgMembers.map(om => <option key={om.user_id} value={om.user_id}>{om.user_id.slice(0, 12)}...</option>)}
              </select></div>
              <div><label className="dl-label">{t("common.role")}</label><select className="dl-input" value={addRole} onChange={e => setAddRole(e.target.value)}>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button onClick={handleAdd} disabled={!selectedUser || adding} className={`dl-btn-primary ${!selectedUser || adding ? 'opacity-40' : ''}`}>{adding ? 'Adding...' : 'Add member'}</button>
              </div>
            </div>
          </DialogContent>
        </Dialog>
