  ...
  const quickActions = [
    {
      ...
      desc: 'Connect a database, upload a CSV, or link a SaaS tool',
      href: `${base}/sources/new`,
      primary: sourceCount === 0,
    },
    {
      icon: BarChart2,
      label: 'View Insights',
      desc: 'AI-generated analysis of your connected data',
      href: `${base}/insights`,
      primary: false,
    },
    {
    ...
    },
    {
      icon: LayoutGrid,
      label: 'Dashboard',
      desc: 'Build a live monitoring dashboard',
      href: `${base}/dashboard`,
      primary: false,
    },
  ]
    ...
    <div className="max-w-4xl mx-auto px-8 py-10">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-mb-2xl font-black text-mb-text-dark">
            {project?.name}
          </h1>
        </div>
        {project?.description && (
          <p className="text-mb-text-medium text-mb-base ml-12">
            {project.description}
          </p>
        )}
      </div>
      ...
      {sourceCount === 0 && (
        <div className="mb-card p-8 text-center mb-8">
          <div className="w-14 h-14 bg-mb-bg-medium rounded-full
          ...
          <p className="text-mb-text-medium text-mb-base mb-6 max-w-sm mx-auto">
            Upload a CSV, connect a database, or link a SaaS tool
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {quickActions.map(action => (
            <button
              ...
              className={`
                text-left p-4 rounded-mb-lg border transition-all
            ...
            >
              <div className={`
                w-8 h-8 rounded-mb-md flex items-center justify-center flex-shrink-0
                ${action.primary ? 'bg-mb-brand' : 'bg-mb-bg-medium group-hover:bg-mb-brand'}
                transition-colors
              `}>
                <action.icon size={16} className={
                  action.primary ? 'text-white' : 'text-mb-text-light group-hover:text-white'
                } />
              </div>
              <div className="flex-1 min-w-0">
                <p className={`text-mb-sm font-black mb-0.5
                  ${action.primary ? 'text-mb-brand' : 'text-mb-text-dark'}`}>
                  {action.label}
                </p>
                <p className="text-mb-xs text-mb-text-medium">{action.desc}</p>
              </div>
              <ArrowRight size={14} className="text-mb-text-light flex-shrink-0 mt-1
