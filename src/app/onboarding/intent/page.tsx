...
export default function IntentPage() {
  ...
  const handleLaunch = async () => {
    ...
    const projectId = localStorage.getItem('datalaser_project_id')

    // Try to save to Supabase so the health page + prepare wizard work
    if (projectId) {
      try {
        ...
        if (!insertErr && inserted) {
          ...
          router.push(`/projects/${projectId}/sources/${inserted.id}/health`)
          return
        }
      } catch (e) {
    ...
    <div className="max-w-2xl mx-auto pt-12 px-6 pb-24">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-mb-bg border-t border-mb-border px-8 py-3 flex items-center justify-between z-40">
        ...
        <button
          ...
          disabled={launching}
          className={`mb-btn-primary px-8 py-2 text-mb-base font-black ${launching ? 'opacity-40 cursor-not-allowed' : ''}`}
        >
          {launching ? 'Saving...' : 'Explore your data'}
        </button>
      </div>
    </div>
  )
