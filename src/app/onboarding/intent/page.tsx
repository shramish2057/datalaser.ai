    ...
    <div className="max-w-2xl mx-auto pt-12 px-6 pb-24">
      ...
      {pageStep === 'configure' && (
        <>
            ...
            <div className="flex flex-wrap gap-2">
              {file?.columns.map(col => (
                <span key={col.name} className={`
                  mb-badge font-mono text-dl-xs
                  ${col.dtype === 'numeric' ? 'dl-badge-info' :
                    col.dtype === 'date' ? 'dl-badge-warning' :
                    col.dtype === 'categorical' ? 'dl-badge-neutral' :
                    'bg-dl-bg-medium text-dl-text-light'}
                `}>
                  {col.name}
            ...
            <div className="grid grid-cols-3 gap-2 mt-2">
              {CHART_TYPES.map(chart => (
                <button
                  ...
                  className={`p-3 rounded-dl-lg border text-left transition-all
                ...
                >
                  ...
                  <div className="text-dl-sm font-bold text-dl-text-dark">{chart.label}</div>
                  <div className="text-dl-xs text-dl-text-light mt-0.5">{chart.desc}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          {error && (
            <div className="px-4 py-3 rounded-dl-md bg-red-50 border border-dl-error text-dl-error text-dl-sm font-bold mb-4">
              {error}
            </div>
          )}
      ...
      {pageStep === 'health' && (
        <div>
          ...
          {pipelineProfile ? (
            <>
              ...
              <div className="dl-card p-6 mb-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <p className="text-dl-xs font-bold text-dl-text-light uppercase tracking-wider mb-1">Quality Score</p>
                    <div className="flex items-center gap-3">
                      <span className="text-4xl font-black text-dl-text-dark">{pipelineProfile.quality_score}</span>
                      <span className="text-dl-text-light text-dl-lg font-bold">/100</span>
                      <span className={`px-2.5 py-1 rounded-full text-dl-xs font-black ${
                ...
                <div className="w-full h-3 bg-dl-bg-medium rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all duration-700 ${
                    pipelineProfile.quality_score >= 90 ? 'bg-dl-success' :
                    pipelineProfile.quality_score >= 70 ? 'bg-yellow-400' :
                    pipelineProfile.quality_score >= 50 ? 'bg-orange-400' : 'bg-dl-error'
                ...
                <div className="mb-6 space-y-2">
                  ...
                  {pipelineProfile.warnings.map((w, i) => (
                    <div key={i} className={`flex items-start gap-3 p-3 rounded-dl-md border ${
                      w.severity === 'red' ? 'border-dl-error bg-red-50' :
                      w.severity === 'amber' ? 'border-orange-300 bg-orange-50' :
                      'border-yellow-300 bg-yellow-50'
                    }`}>
                      {w.severity === 'red' ? <XCircle size={14} className="text-dl-error flex-shrink-0 mt-0.5" /> :
                      ...
                      <div>
                        ...
                        {w.affected_rows != null && (
                          <span className="text-dl-xs text-dl-text-light ml-1">({w.affected_rows.toLocaleString()} rows)</span>
                        )}
                      </div>
                    </div>
                  ))}
            ...
            error && (
              <div className="mb-6 p-4 rounded-dl-md bg-orange-50 border border-orange-300 flex items-start gap-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <button onClick={goToClean} disabled={!savedSourceId}
                className={`text-left p-5 rounded-dl-lg border transition-all group
                  ${pipelineProfile && pipelineProfile.warnings.length > 0
                    ? 'border-dl-brand bg-dl-brand-hover'
                    : 'border-dl-border hover:border-dl-brand hover:bg-dl-brand-hover'}
                  ${!savedSourceId ? 'opacity-40 cursor-not-allowed' : ''}`}>
                <div className={`w-10 h-10 rounded-dl-md flex items-center justify-center mb-3 transition-colors
                  ${pipelineProfile && pipelineProfile.warnings.length > 0
                    ? 'bg-dl-brand' : 'bg-dl-bg-medium group-hover:bg-dl-brand'}`}>
                  <Wand2 size={18} className={pipelineProfile && pipelineProfile.warnings.length > 0
                    ? 'text-white' : 'text-dl-text-light group-hover:text-white'} />
                </div>
