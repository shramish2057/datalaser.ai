...
export default function CellCard({ cell, cellNumber, isActive, isFirst, isLast, onRun, onCodeChange, onContentChange, onTypeChange, onLevelChange, onDelete, onMoveUp, onMoveDown, onSaveToLibrary, onClick }: Props) {
  ...
  if (cell.type === 'heading') {
    const sizes = { 1: 'text-[22px] font-bold', 2: 'text-[18px] font-semibold', 3: 'text-[15px] font-semibold text-dl-text-medium' }
    return (
    ...
    return (
      <div onClick={onClick} className={`mb-2 group relative ${isActive ? 'ring-1 ring-dl-brand rounded-dl-md' : ''}`}>
        ...
        <textarea value={cell.content || ''} onChange={e => { onContentChange(e.target.value); e.target.style.height = 'auto'; e.target.style.height = e.target.scrollHeight + 'px' }}
          ...
          className="w-full bg-transparent border-none outline-none text-dl-text-dark px-4 py-2 text-[14px] leading-[1.8] resize-none min-h-[60px]" />
      </div>
    )
  }
  ...
  return (
    <div onClick={onClick} className={`bg-white border rounded-dl-lg mb-2 cursor-pointer transition-all
      ${isActive ? 'border-dl-brand shadow-sm' : 'border-dl-border hover:border-dl-border-dark'}`}>
      <div className="flex items-center justify-between px-2 py-1.5 border-b border-dl-border">
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="relative">
            <button onClick={e => { e.stopPropagation(); setShowMenu(!showMenu) }} className="p-1 rounded hover:bg-dl-bg-medium">
              <MoreVertical size={13} className="text-dl-text-light" />
            </button>
            {showMenu && (
              <div className="absolute right-0 top-full z-10 bg-white border border-dl-border rounded shadow-sm py-1 w-40" onClick={e => e.stopPropagation()}>
                {!isFirst && <button onClick={() => { onMoveUp(); setShowMenu(false) }} className="w-full text-left px-3 py-1.5 text-[12px] text-dl-text-dark hover:bg-dl-bg-light flex items-center gap-2">
                  <ArrowUp size={12} /> {t('studio.moveUp')}
                </button>}
                {!isLast && <button onClick={() => { onMoveDown(); setShowMenu(false) }} className="w-full text-left px-3 py-1.5 text-[12px] text-dl-text-dark hover:bg-dl-bg-light flex items-center gap-2">
                  <ArrowDown size={12} /> {t('studio.moveDown')}
                </button>}
                <button onClick={() => { onSaveToLibrary(); setShowMenu(false) }} className="w-full text-left px-3 py-1.5 text-[12px] text-dl-text-dark hover:bg-dl-bg-light flex items-center gap-2">
                  <BookOpen size={12} /> {t('studio.saveToLibrary')}
                </button>
                <div className="h-px bg-dl-border my-1" />
                <button onClick={() => { onDelete(); setShowMenu(false) }} className="w-full text-left px-3 py-1.5 text-[12px] text-dl-error hover:bg-red-50 flex items-center gap-2">
