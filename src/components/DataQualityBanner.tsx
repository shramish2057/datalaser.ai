...
export function DataQualityBanner({ report, prepareUrl }: { report: DataQualityReport; prepareUrl?: string }) {
  ...
  if (report.level === 'good') {
    return (
      <div className="flex items-center gap-2 px-4 py-2.5 rounded-dl-lg bg-green-50 border border-green-200 mb-6">
        <CheckCircle size={15} className="text-dl-success flex-shrink-0" />
        <span className="text-dl-sm font-bold text-green-700">Data quality: Good</span>
        <span className="text-dl-sm text-green-600 ml-1">&mdash; {report.summary}</span>
        <span className="ml-auto text-dl-xs font-bold text-green-600">Score: {report.score}/100</span>
      </div>
    )
  }
  ...
  return (
    <div className={`rounded-dl-lg border ${c.bg} ${c.border} mb-6 overflow-hidden`}>
      {/* Header row */}
      <div
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <span className={`text-dl-sm font-black ${c.title}`}>
            {report.level === 'yellow' ? 'Minor data quality issues' :
             report.level === 'amber' ? 'Data quality issues detected' :
             'Significant data quality issues'}
          </span>
          <span className={`text-dl-sm ${c.text} ml-2`}>&mdash; {report.summary}</span>
        </div>
        <span className={`text-dl-xs font-bold px-2 py-0.5 rounded-full ${c.badge} flex-shrink-0`}>
          Score: {report.score}/100
        </span>
        <span className={`text-dl-xs font-bold ${c.text} flex items-center gap-1 flex-shrink-0`}>
        ...
        <div className={`border-t ${c.border} px-4 py-3`}>
          ...
          <div className="space-y-2 mb-3">
            {report.warnings.map((w, i) => (
              <div key={i} className="flex items-start gap-3">
                <span className={`
                  text-dl-xs font-bold px-1.5 py-0.5 rounded flex-shrink-0 mt-0.5
                ...
                <div>
                  <span className={`text-dl-sm font-bold ${c.title}`}>{w.column}</span>
                  <span className={`text-dl-sm ${c.text} ml-1`}>
                    &mdash; {issueLabels[w.issue]}: {w.detail}
                  </span>
                  {w.affectedRows != null && (
                    <span className={`text-dl-xs ${c.text} ml-1`}>
          ...
          {prepareUrl && (
            <div className={`flex items-center justify-between pt-3 border-t ${c.border} mb-3`}>
              <span className={`text-dl-xs font-bold ${c.text}`}>
                Fix these issues with the data preparation pipeline
              </span>
              <a
                href={prepareUrl}
                className="dl-btn-primary text-dl-xs px-3 py-1.5 flex items-center gap-1 ml-4 flex-shrink-0"
              >
                <Wand2 size={12} />
                Clean this data
              </a>
          ...
          {report.upgradePrompt && (
            <div className={`flex items-center justify-between pt-3 border-t ${c.border}`}>
              <span className={`text-dl-xs ${c.text}`}>{report.upgradePrompt}</span>
              <button className="dl-btn-primary text-dl-xs px-3 py-1.5 flex items-center gap-1 ml-4 flex-shrink-0">
          ...
          {report.level === 'red' && report.canProceed && (
            <p className={`text-dl-xs ${c.text} mt-2`}>
