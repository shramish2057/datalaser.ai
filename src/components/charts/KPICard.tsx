...
export function KPICard({ label, value, delta, deltaLabel, unit = '', trend, size = 'md', sparkline, sparklineColor }: Props) {
  ...
  const trendColor = autoTrend === 'up' ? 'text-green-600' : autoTrend === 'down' ? 'text-red-500' : 'text-dl-text-light'
  ...
  return (
    <div className="bg-white border border-dl-border rounded-dl-lg p-4 flex flex-col">
      <span className={`${labelSizes[size]} font-semibold uppercase tracking-wider text-dl-text-light mb-1`}>
        {label}
      </span>
      <div className="flex items-end justify-between gap-3">
        <div>
          <div className="flex items-baseline gap-1.5">
            <span className={`${valueSizes[size]} font-black text-dl-text-dark leading-none`}>
              {formattedValue}
            </span>
            {unit && <span className="text-[12px] text-dl-text-light">{unit}</span>}
          </div>
          {delta !== undefined && (
