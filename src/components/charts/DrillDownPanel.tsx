  ...
  return (
    <div className="fixed inset-y-0 right-0 w-[520px] bg-white shadow-2xl border-l border-dl-border z-50 flex flex-col animate-in slide-in-from-right duration-200">
      ...
      <div className="px-4 py-2 border-b border-dl-border flex items-center gap-1.5 flex-wrap">
        <span className="text-[11px] text-dl-text-light">All</span>
        {filters.map((f, i) => (
          <span key={i} className="flex items-center gap-1">
            <ChevronRight size={10} className="text-dl-text-light" />
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-dl-brand-hover text-dl-brand text-[11px] font-medium">
              {f.label}
      ...
      {numericStats.length > 0 && (
        <div className="px-4 py-2 border-b border-dl-border">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-dl-text-light mb-1.5">Subset Stats</p>
          <div className="grid grid-cols-3 gap-2">
            {numericStats.slice(0, 3).map(s => (
              <div key={s.col} className="text-center">
                <p className="text-[10px] text-dl-text-light truncate">{s.col}</p>
        ...
        <table className="w-full text-[11px]">
          <thead className="sticky top-0 bg-white">
            <tr className="border-b border-dl-border">
              {columns.map(c => (
                <th key={c} className="px-2 py-1.5 text-left text-[10px] font-bold text-dl-text-light uppercase tracking-wider whitespace-nowrap">
                  {c}
                </th>
              ))}
            </tr>
      ...
      <div className="h-[40px] flex items-center justify-between px-4 border-t border-dl-border flex-shrink-0">
        <div className="flex items-center gap-2">
          <button onClick={() => setPage(Math.max(0, page - 1))} disabled={page === 0}
            className="text-[11px] text-dl-text-medium hover:text-dl-brand disabled:opacity-30">
            ← Prev
          </button>
          <span className="text-[11px] text-dl-text-light">
        ...
        {onOpenInStudio && (
          <button onClick={onOpenInStudio}
            className="flex items-center gap-1 text-[11px] text-dl-brand hover:underline">
